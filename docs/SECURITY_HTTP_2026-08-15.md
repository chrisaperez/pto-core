...
per job and a socket inside that task is a way around it. An orchestrator-side
allowlist can refuse to dispatch `serve`, but that is a policy several
deploys away from the binary, and it is not true of the artefact itself.
...
* A worker image's own Dockerfile can set the flag and then **assert the
  property of the image it just built**, failing the build if `serve` is
  accepted. A flag can be dropped in a rebase; the assertion is what survives
  it.
