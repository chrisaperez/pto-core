...
token); a caller that validates window size upstream might cap it well inside
this range and never reach it. **Fix:**
...
A caller validating these same parameters upstream (window ≤ 1,000,000 bp, bin
1–100,000, MAPQ 0–255, non-negative fragment and extension), and a workflow
manager passing its own allocated thread count, both sit inside the new
ranges.
