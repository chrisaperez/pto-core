...
| X1 | **High** (`pto-cloud`) | Multi-shard `fastq_qc` merge reports shard 0's counts as the job's | Fixed in a separate `pto-cloud` change, not in this commit |
...
`pto-cloud`'s merge (X1) already marks such floats "unmergeable; recompute
from summed counters", and says the summary does not carry those counters.
...
### X1 — `pto-cloud` merges only shard 0 of a split `fastq_qc` job (High, not fixed)

- **Where.** `pto-cloud/orchestrator/gather.py` appends each shard's parsed
  `fastq_stream` JSON. `orchestrator/merge.py::_merge_fastq_qc` sums top-level
  integers and numeric lists. Every nested object (`summary`, `filtering`,
  `per_cycle`, `input`, …) falls to `out[key] = head`, the first shard's value.
- **Splitting is allowed.** `partition.py` lists `fastq_qc` in `DECOMPOSABLE`.
- **Reproduction.** Two real shard reports (500 and 1,500 reads) from this
  build merged to `summary.reads_in` 500 against 2,000 for the whole-file run,
  and `filtering.quality_trimmed` 500 against 2,000. `unmergeable` was empty,
  so nothing flagged it. Only `quality_histogram`, a top-level list, summed
  correctly.
- **Why tests missed it.** `tests/test_partition.py` merges flat hand-written
  payloads (`{"reads": …, "per_cycle_q": …}`), not the tool's schema.
- **Status.** Fixed in a separate `pto-cloud` change (not part of the
  `fastq_stream` commit), under that project's verification contract.
  `orchestrator/merge.py::_merge_fastq_qc` now merges by the report's schema:
  - counters are summed;
  - rates are recomputed from A5's numerators;
  - the histogram and per-cycle base counts are summed element-wise;
  - invariants (`tool`, `version`, `input.format`, `phred_offset`,
    `cycle_limit`) must agree or the merge raises;
  - per-cycle means, per-task details and unknown fields come back `None`
    with a reason.

  `tests/test_fastq_qc_merge.py` checks it against reports the binary wrote
  (checked-in fixtures plus a live-binary run). Against the old merge the new
  tests fail 11 of 12; `make verify` passes, and `make test` ran 626 passed,
  29 skipped.
