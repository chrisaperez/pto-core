...
fragments, not read pairs. No in-repo caller is affected — this repository's
own wheel API and `tests/validation` pass no fragment BED with a count column
(checked by search); an external caller that does would see the new default.
**Tests:** `gtk_fragment_stream`
...
A caller validating these same bounds upstream (lengths 0–1,000,000, MAPQ
0–255, genome size ≤ 10^11) sits inside the new ranges, so nothing it accepts
would be refused by the binary; this repository's own wheel API passes values
through unbounded by design.
