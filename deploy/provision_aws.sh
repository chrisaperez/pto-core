...
# group rule that would. cuttag_profiler requires a session token on every
# /api/* request, but it is one shared secret for the whole run, not a
# per-user login system, and the Host/Origin checks alone only bind a browser,
# not anything that can open a raw socket.
#
...
$(printf '\033[1;33m')SECURITY GROUP$(printf '\033[0m')
  ...
  Do NOT add an inbound rule for ${PORT}. cuttag_profiler requires a session
  token on every /api/* request, but it is one shared secret for the whole
  run and not a login system, so an internet-reachable ${PORT} would let
  anyone who obtains that token profile, download and enumerate every file
  under the data root. H2 is closed, so that is now bounded by the mount
