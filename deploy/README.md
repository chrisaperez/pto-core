...
own CUT&Tag/CUT&RUN signal matrices. It is **not** the managed multi-tenant
service (that is `pto-cloud`, a separate project, and it never runs `serve` at
all — see `pto-core/CMakeLists.txt`'s `PTO_CLOUD_BUILD` option, which compiles
the server out of the binary entirely for that build).
