...
that is deliberate for the same reason `pto-cloud/tests/test_integration_e2e.py`
calls `POST /v1/jobs` for real: a fake that returns what we believe the tool
prints cannot catch a change in what it actually prints, and the argv contract is
...
def test_a_path_containing_shell_metacharacters_is_passed_through_intact(tmp_path):
    """argv, never a shell -- job_specs.py rule 1, and it is not weaker locally.
