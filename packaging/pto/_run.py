...
1. ARGV, NEVER A SHELL. Every invocation is a list handed to
   ...
   into a command line. This is not weaker on a workstation than anywhere
   else: `cuttag_profiler`'s SECURITY_HTTP L8 finding was a `--host` value
   reaching `std::system()`, on a local tool.
...
def run_tool(
    ...
    *,
    ...
    tools genuinely differ in how they report; this function just captures
    stdout for one and collects the named file for the other.
    """
