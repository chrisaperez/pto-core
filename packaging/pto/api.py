...
The alternative -- mirroring the defaults in these signatures, as
`api/app/job_specs.py` does -- would put a fourth copy of every bound and
default in the tree, after the binary's own CLI parser, `common/models.py`, and
the generated TypeScript in pto-studio. This repository has been bitten by that
exact defect twice in two languages (`pto-core`'s REVIEW_2026-08-15 finding 2;
`common/models.py`'s own header), and `pto-cloud/tests/test_shared_models.py`
exists to stop a third. A copy here would be the third, and it would be the
one nobody diffs, because it lives in a different half of the monorepo from
the guard that watches for it.

The cloud API is right to restate them -- it validates before provisioning a
Fargate task and pulling 90 GB, so a bad value must be a 422 rather than a
billed failure. That economic argument does not exist here. The binary is on
...
layer, and it is why this package takes no dependency on `pto-cloud`.
...
def profile(
    ...
    *,
    ...
    `serve`, the tool's interactive dashboard, is deliberately unreachable from
    this package. It is a single-tenant loopback tool whose session token is
    bound to one operator's browser; a library call that started a listening
    socket on a shared cluster node would be the wrong default even where the
    binary still has one compiled in. Render `profile.tsv` instead.
    """
