...
from a deterministic property of the input's shape: a naming mismatch, an
out-of-order record, every region skipped. Any caller that retries failures
automatically -- a job scheduler, or a workstation `for` loop over 300
samples -- needs the same distinction, and the same answer: retrying an
`InvalidInput` reaches the same answer, more slowly. `fastq_stream` is the one
CLI where `InvalidInput` also covers a plain usage error (`main.cpp:82-85`),
not only the input's shape.
"""
