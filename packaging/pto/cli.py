...
NAMED `pto-core`, NOT `pto`, and that is not a style choice: the `pto` script
name on PyPI already belongs to another distribution. Two distributions
installing the same script name means whichever was installed second wins,
silently. The import name `pto` is free and is what the Python API uses; the
script name is not.
