...
Thread count for `fastq_stream` and `cuttag_profiler` is passed explicitly,
from `pto.available_cpus()`, which takes the **minimum** of CPU affinity, the
cgroup CPU quota and `os.cpu_count()`. Left alone, `std::thread::hardware_concurrency()`
...
`threads=` to override, on either of those two tools.

`genomic_toolkit` takes no thread flag at all: its OpenMP reduction reads
`OMP_NUM_THREADS`, which this wrapper leaves unset. `pto-peaks` is
single-threaded and has no thread option.
...
The script is `pto-core`, not `pto`: the `pto` name on PyPI already belongs to
...
## The dashboard is not here (mostly)

`cuttag_profiler serve` — the interactive HTML dashboard — is not reachable
through `pto.profile(...)` or `pto-core profile`: neither one ever builds a
`serve` argv. Its authentication is a session token bound to one operator's
browser, which is sound on a workstation and the wrong shape on a shared
cluster node, so that is deliberate. Use `pto.profile(...)` and render
`profile.tsv`.

It is still reachable another way: `pto-core exec cuttag_profiler serve
--data-root DIR` passes straight through, because `exec` has no subcommand
allowlist, and the wheel's `cuttag_profiler` binary is built with the server
compiled in. Nothing but not running that command stops it today.
