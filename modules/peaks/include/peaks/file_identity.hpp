...
//
// pto-peaks: would writing an output path destroy an input?
//
// pto-peaks opens its output only after the last record is read, so it never
// truncated its input before reading it. It did something no better: with
// `-o fragments.bed fragments.bed`, or `-o hg38.chrom.sizes`, or
// `-o x.bed < x.bed`, the run exited 0 and the input was replaced by the
// narrowPeak it produced (docs/TORTURE_2026-09-10.md, peaks P1).
//
