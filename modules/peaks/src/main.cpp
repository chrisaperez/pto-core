...
// browser reads.
//
// Exit codes follow the repository's convention: a job scheduler or pipeline
// should retry a 1, never a 2:
//
...
int run(const Options& opts) {
  ...
  if (looks_like_bam) {
    if (!BamStreamer::available()) {
      ...
      // One exit code per input regardless of build flags is what a caller's
      // own retry rule needs to be able to assume.
      std::fprintf(stderr,
