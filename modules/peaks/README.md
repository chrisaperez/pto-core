...
```bash
pto-peaks atac.bam peaks.narrowPeak
```
...
Exit 2 is never retried. Both `pto-cloud`'s job contract and
`nextflow/conf/pto.config` key off that convention, and they key off it because
a retry of a deterministic wrong-shape failure spends the job's timeout arriving
at the same answer.
