...
//
// cuttag_profiler: would writing an output path destroy an input?
//
// `cuttag_profiler profile --out-matrix regions.bed` replaced the region file
// with the matrix computed from it; `--out-profile x.bam.bai` destroyed the
// index every later run needs; and `--out-matrix f --out-profile f` wrote one
// output over the other. Same defect class as docs/TORTURE_2026-09-10.md F1.
//
