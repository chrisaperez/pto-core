...
// there. An orchestrator-side allowlist can also refuse to invoke `serve`,
// but that is a policy several deploys away from the binary; this compiled-
// out path is the one that survives a bad deploy.
#include <algorithm>
#include <charconv>
#include <cstdio>
#include <cstdlib>
...
int run_profile(int argc, char** argv) {
    ...
    // answer wrong" status, which a caller should never retry.
