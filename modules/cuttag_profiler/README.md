...
owns it. One is created only for an explicitly single-threaded caller, where it
can still overlap inflation within a single query. Rather than merely adding
threads wherever a library offers them, the profiler places parallelism at the
level where the work actually divides.
