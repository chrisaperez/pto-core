...
//
// Buffers are recycled through a free list behind a std::mutex. That is not on
...
class Buffer {
 public:
  ...
  // Set by the reader as chunks are produced. Currently write-only downstream
  // -- no stage reads it back to reorder. Order instead comes from lane
  // topology: chunk k always rides lane k mod D (or k mod W), and collectors
  // read lanes round-robin in that same order.
  uint64_t seq = 0;
