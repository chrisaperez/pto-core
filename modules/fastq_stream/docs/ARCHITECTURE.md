...
Default split with `--threads N`: `spare = max(1, N-3)`, `D = max(1, spare/3)`
(BGZF only), `W = spare - D`. The `max(1, ...)` floor means BGZF always gets at
least one inflater: `-t 1..5` all spawn one, not zero. The reader, assembler
and writer always exist, so `--threads 4` yields one worker. The text report
always prints the real breakdown — see §9.
...
- `head_` and `tail_` sit on separate cache lines. The padding is a fixed 128
  bytes on arm64 and x86-64, 64 bytes elsewhere.
  `std::hardware_destructive_interference_size` is deliberately not used: it is
  an implementation-defined constant with no cross-TU stability guarantee, and
  mixing two values for it across translation units is an ODR hazard.
...
`include/fastq_stream/simd.hpp` dispatches at runtime: each kernel below is
compiled in AVX-512BW, AVX2, NEON and scalar variants behind per-function
`target` attributes, and `__builtin_cpu_supports` picks one the first time it
runs and caches the choice. `FQ_FORCE_ISA` overrides it, to any variant
compiled in. (`-DFQ_NATIVE=ON` separately uses `-march=native` /
`-mcpu=native` for the rest of the translation unit; it does not change how
these kernels dispatch, and defaults OFF so a wheel build stays portable.)
...
\* `sum_u8` has no production caller today — `phred_calculator.hpp` accumulates
mean Phred inline instead — so it is exercised only by its own tests and the
benchmark.
...
totals once per chunk. The bound is the smallest record, not a typical one: a
320 KiB chunk of 1-base records holds ~43,000 of them, and a per-cycle sum
over that many stays below 43,000 × 126 = 5.4e6 — so the inner loop stays in
32-bit lanes with no overflow risk (`phred_calculator.hpp:6-12`).
...
6. **Scaling saturates.** Measured 645 / 1863 / 3063 / 4304 MB/s at 1 / 3 / 5 / 9
   ...
   reverses the gains.
7. **BAM/CRAM/SAM input is opt-in**, not on by default in this module
   (`-DFQ_ENABLE_HTS=ON`; the wheel builds with it on). It is read through
   htslib's own parser, not the BGZF layer above, and is recognised only in a
   regular file — never on stdin.
