...
//
// genomic_toolkit: would writing an output path destroy an input?
//
// `genomic_toolkit sizes x.bam --histogram x.bam` would open the histogram with
// "w" after streaming the BAM and replace it with a two-column table; so would
// `--histogram peaks.bed` on a frip run, or `--histogram x.bam.bai`, which
// destroys the index a --region query needs. Same defect class as
// docs/TORTURE_2026-09-10.md F1 in fastq_stream.
//
