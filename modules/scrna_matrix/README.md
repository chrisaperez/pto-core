...
* Exact brute-force k-NN through a bounded top-k heap kept per row -- O(n·k)
  total, and not a function of thread count, because the symmetric tile sweep
  needs every row's heap alive across all rounds -- and row norms hoisted out
  of the O(n^2) loop.
