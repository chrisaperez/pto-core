...
#
# A session token is required on every /api/* request (printed above, and by
# the binary on startup), on top of the Host/Origin checks -- but it is one
# shared secret for this run, not a login system. Keep this bound to loopback.
# Never pass --allow-remote on a shared or internet-facing host -- see
# docs/DEMO.md for the tunnel pattern.
